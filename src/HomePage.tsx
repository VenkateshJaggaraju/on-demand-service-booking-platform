...
export const HomePage: React.FC = () => {
  ...
  // Whether *this* browser currently holds a valid customer session.
  // "checking" avoids a flash of the wrong buttons before /auth/validate
  ...
  // Runs once on mount so the navbar reflects reality immediately instead
  // of always defaulting to "Login / Sign Up".
  useEffect(() => {
    let mounted = true;

    isCustomerLoggedIn().then((loggedIn) => {
      if (mounted) setAuthStatus(loggedIn ? "loggedIn" : "loggedOut");
    });
  ...
  // Fixes: navbar "Login" / "Sign Up" always went to /customer/login even
  // when the person already had a valid session, because those were plain
  ...
  const renderAuthButtons = (onNavigate?: () => void) => {
    if (authStatus === "checking") {
      // Render nothing rather than flashing "Login" for a split second.
      return null;
    }
      ...
      return (
        <>
          <Link to="/services" onClick={onNavigate}>
            <button className="btn btn-outline">My Services</button>
          </Link>
    ...
    return (
      <>
        <Link to="/customer/login" onClick={onNavigate}>
          <button className="btn btn-outline">Login</button>
        </Link>
        <Link to="/customer/register" onClick={onNavigate}>
        ...
        {mobileMenu && (
          <div className="mobile-nav">
            ...
            <div className="mobile-auth">
              {renderAuthButtons(() => setMobileMenu(false))}
            </div>
          </div>
        )}
      </header>
